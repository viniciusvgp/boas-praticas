...
* #1.3 Remota: Instalação do =MPI=                                   :noexport:
...
Agora crie um script para instalar as versões de OpenMPI no parque.

Nome do arquivo sugerido =instala-openmpi.slurm=
  #+begin_src shell :tangle instala-openmpi.slurm
#!/bin/bash
#SBATCH --nodes=1
...
Em seguida, no parque computacional, submeta o job:

#+begin_src shell :results output
sbatch instala-openmpi.slurm
#+end_src

Perceba o número de JOBID fornecido pelo slurm, e acompanhe a compilação com:

#+begin_src shell :results output
tail -f slurm-JOBID.out
#+end_src

*Atenção*: a instalação leva aproximadamente 20 minutos com 8 cores.
