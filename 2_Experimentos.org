...
1, 10 e 20, e dois tamanhos 3000 e 6000, para um total de 6
...
while IFS=, read -r name runnoinstdorder runno runnostdrp \
	 processes mpi Blocks
do
    # Limpar valores
    export name=$(echo $name | sed "s/\"//g")
    export processes=$(echo $processes | sed "s/\"//g")
    export mpi=$(echo $mpi | sed "s/\"//g")

    # Definir uma chave única
    KEY="$name-$processes-$mpi"

    # Usar a versão apropriada de MPI
    MPICC=$(spack location -i openmpi@$mpi)/bin/mpicc
    MPIRUN=$(spack location -i openmpi@$mpi)/bin/mpirun
