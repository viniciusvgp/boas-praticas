...
dados. A aplicação ~BT-MZ~ gera como saída arquivos texto no formato
abaixo:
...
parâmetros utilizados na execução e as verificações de erro. Para
efeitos de demonstração, neste tutorial estamos interessados apenas no
tempo de execução. Dessa forma, faremos uma limpeza nos arquivos de
~log~, de forma a mantermos apenas a informação referente ao tempo de
execução.
...
Como demonstração, faremos algumas análises estatísticas básicas
