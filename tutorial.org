...
comando ~sbatch~. Este /script/ exemplo solicita a reserva de 2 nós na
plataforma =hype= pelo período máximo de 40 minutos. As saídas padrão
(/stdout/) e de erro (/stdin/) serão redirecionadas para arquivos nomeados
com o identificador do /job/.
...
*** Execução Não-Iterativa de Experimentos com SLURM
* Análise de Dados
# R + tidyverse
